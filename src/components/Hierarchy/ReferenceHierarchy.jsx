...
const Reference = () => {
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    const requestData = {
      referenceUserId: values.referenceUserId,
      emails: values.emails.map((email) => email.value).join(","),
      projectIds: values.projectIds.map((project) => project.value).join(","),
      updateHierarchy: values.updateHierarchy,
    };
    ...
    try {
      const response = await axios.post(
       `${Api_base_url}/api/project/add-users`,
        requestData,
        {
          headers: {
            "Content-Type": "application/json",
        ...
        <Card.Body className="card-1-form d-block">
          <Formik
            initialValues={{
              projectIds: [],
              emails: [],
              referenceUserId: "",  // Added this field
            ...
            {({ values, setFieldValue, isSubmitting, resetForm }) => (
              <Form>
                <div className="row">
                  ...
                  <div className="col-md-4 mb-4">
                    <label className="form-label">Reference User</label>
                    <Field
                      type="number"
                      name="referenceUserId"
                      className="form-control"
                      placeholder="Enter Reference User"
                    />
                  </div>
                  <div className="col-md-4 mb-4">
                    <label className="form-label">Updated Hierarchy</label>
                    <Field
                      type="number"
